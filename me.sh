...
function gpscript_palette() {
	...
	VIRIDIS="viridis"
	GREEN="defined (0 '#8CB402', 1 '#ECFAD5', 2 '#A5E49D', 3 '#8CCE72', 4 '#69B84A')"
...
function gnuplot_gpval() {
    ...
    function ticlength(max, min, interval) {
		...
		if (max ~ /\./) {
			max2 = max < 0 ? substr(max,1,length(interval)+1) : substr(max,1,length(interval))
		} else {
			max2 = max
		}
		if (min ~ /\./) {
		...
		max_minus = max2 - interval
		min_plus = min2 + interval
		tic = length(max_minus) > length(max2) ? max_minus : max2
		tic = length(min_plus) > length(tic) ? min_plus : tic
		tic = length(min2) > length(tic) ? min2 : tic
        t_dot = gsub(/\./, "", tic)*0.5
    ...
    }
	{
		...
		switch ($1) {
			case "GPVAL_LAST_PLOT":
				...
				gsub(/0+$/,"",$18); gsub(/\.$/,"",$18); Xsize[x][y] = $18; Xnum[$18]++
				gsub(/0+$/,"",$20); gsub(/\.$/,"",$20); Ysize[x][y] = $20; Ynum[$20]++
                Ylabel[x][y] = $24
	...
	for key in ${!Key[*]}; do
		if [[ ${key%,*} == $1 && ${Key[$key]} != "" ]]; then
			...
			if [[ ${fs//[0-9]/} == "" ]]; then
				...
				Usrfontset="font \",$(awk "BEGIN {print $Fontsize-3}")\""
			fi
			echo "set key at graph ${p3%,*} ${p3##*,} ${Key_layout[$1]} Left reverse samplen 1 spacing 1.25 noautotitle $Usrfontset" >> .me/gp
...
function gpscript_set_axis() {
	...
	case $Mgraph in
		2d)	xl_pos="0,"$(awk "BEGIN {printf \"%.2f\", 0.425/$Labelmargin}")"■center" 
			...
			xt_pos="0,0.25"
			yt_pos="0.29,0"
			vs1=5.26; hs1=$(awk "BEGIN {printf \"%.2f\", ${GPV[hspace]}-1}");;
	...
	case $2 in
		...
		cmr10) sed -e 's|{s/\\infty}|{w/\\061}|g
			...
			sed -e 's|{//|{/cmti10 |g
				...
				s|{s//|{/cmmi10 |g
				s|{w/\([^/]\)|{/cmsy10 \1|g
				/label/s|\||{/cmsy10 \\152}|g
				/label/s|\|\||{/cmsy10 \\153}|g
